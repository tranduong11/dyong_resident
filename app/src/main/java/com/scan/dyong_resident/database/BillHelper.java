...
public class BillHelper extends SQLiteOpenHelper {
    ...
    public Bill getBillByRoom(String roomNumber) {
        if (roomNumber == null || roomNumber.isEmpty()) {
            // Trả về null nếu phòng không hợp lệ
            Log.e("BillHelper", "Room number is null or empty");
            return null;
        }
        ...
        try {
            cursor = db.query(TABLE_BILL, null, COLUMN_PHONG + " = ?",
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                bill = new Bill(id, phong, chuHopDong, kyThanhToan, ngayLap, hanThanhToan,
                        soLuong, tienPhong, tienDien, tienNuoc, tienMang, tienThangMay, tienRac);
            }
        } finally {
            if (cursor != null) cursor.close();
